...
async function fixProductCategories() {
  ...
  for (const product of products) {
    ...
    if (!categoryExists) {
      ...
      correctCategory = categories.find(
        (c) => c.nombre.toLowerCase() === lowerCategoria
      );
      ...
      if (!correctCategory) {
        if (lowerName.includes("corona")) {
          correctCategory = categories.find(
            (c) => c.nombre.toLowerCase() === "coronas"
          );
        } else if (lowerName.includes("cruz")) {
          correctCategory = categories.find(
            (c) => c.nombre.toLowerCase() === "cruces"
          );
        } else if (lowerName.includes("arco")) {
          correctCategory = categories.find(
            (c) => c.nombre.toLowerCase() === "arcos"
          );
        }
      }
